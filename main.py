...
weatherColors = {
  ...
  314: discord.Color.from_rgb(176, 196, 222),  # Heavy Shower Rain and Drizzle
...
def help():
  ...
  embed.add_field(
    ...
    value=
    "Get specific weather information for a city. Specify the required fields to display.",
    inline=False)
  embed.add_field(
    name="`w!convert <unit>`",
    value=
    "Convert temperature unit to Celsius (c), Fahrenheit (f), or Kelvin (k).",
    inline=False)
  embed.add_field(
    name="Note",
    value=
    "* Angular brackets are just for syntax purposes\n* None of these commands are case sensitive",
    inline=False)
  return embed
...
async def on_message(message):
  ...
  if (mes.split()[0] == "w!command"):
    ...
    if (command == "w!getatm" or command == "getatm"):
      ...
      embed.add_field(name="Syntax: `w!getAtm <city_name>`",
                      value="",
                      inline=False)
      embed.add_field(name="Example: `w!getAtm Tokyo`", value="", inline=False)
    ...
    if (command == "w!get" or command == "get"):
      ...
      embed.add_field(
        name=
        "Example: `w!get Los Angeles timezone, current temperature/current, forecast, humidity, weather`",
        value="",
        inline=False)
  ...
  if (mes.split()[0] == "w!get"):
    ...
    attributes = spli[1].split(",")
    city = attributes[0]
    attributes = attributes[1:]
    cityli = city.split()
    attributes.insert(0, cityli[-1])
    cityli = cityli[:-1]
    city = " ".join(cityli)
    if (city != ""):
      try:
